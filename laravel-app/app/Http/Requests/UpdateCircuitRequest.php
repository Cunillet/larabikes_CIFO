...
class UpdateCircuitRequest extends FormRequest
{
    ...
    {
        $circuit = Circuit::find($this->circuit);
        return $this->user()?->can('update', $circuit) ?? false;
    }

    protected function failedAuthorization()
    {
        throw new AuthorizationException('Unable to edit a circuit if not admin');
    }
    ...
    {
        return [
            'name' => ['required', 'string', 'max:256', Rule::unique('circuits')->ignore($this->circuit)],
